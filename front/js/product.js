...
function conditionnalStorage(selectColor, inputQuantity, storage) {
    ...
    if (productIndex !== -1) {
       let newQuantity =  products[productIndex].inputQuantity + storage.inputQuantity
       if (newQuantity > 100 || newQuantity < 1) {
        alert('Veuillez choisir une quantité entre 1 et 100')
       } else {
            newQuantity + products[productIndex].inputQuantity
            products[productIndex].inputQuantity = newQuantity 
       }
    } else if (selectColor === "" || inputQuantity < 1) {
